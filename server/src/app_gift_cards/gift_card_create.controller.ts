...
import { fulfillGiftCardItems, giftCardCreate, orderQuery } from "./gift_card_queries.controller";
import { calculateOneYearFromNow, generateGiftCardCode } from "./gift_card_utils.controller";
...
export const gift_card_create = async (req: Request, res: Response) => {
  try {
    ...
    const orderStatusUrl = req.query.store;
    console.log("Order status URL:", orderStatusUrl);
    // ======= MOCK DATA =======
    // orderStatusUrl = "https://potzillas-dev.myshopify.com";
    // ======= END MOCK DATA =======

    const stores = req.query.test == "1" ? get_dev_stores(orderStatusUrl) : get_stores(orderStatusUrl);

    if (!stores) {
      console.log("GIFT CARD CREATE: Store not found");
      return res.status(200).json({ message: "GIFT CARD CREATE: Store not found" });
    }
    ...
    const customerId = "gid://shopify/Customer/" + req.body?.customer?.id;
    ...
    const totalGiftCardsToCreate = giftCardItems.reduce((acc: any, item: any) => {
      return acc + item.quantity;
    }, 0);

    const giftCardsOfTheSameOrderInDB = await GiftCard.findAll({
      where: {
    ...
    if (giftCardsOfTheSameOrderInDB.length >= totalGiftCardsToCreate) {
      console.log(`GIFT CARD CREATE: Gift cards for order ${orderName} already exist`);
      return res.status(200).json(`GIFT CARD CREATE: Gift cards for order ${orderName} already exist`);
    }
    ...
    for (const [index, cardItem] of giftCardItems.entries()) {
      for (let qtyIndex = 0; qtyIndex < cardItem.quantity; qtyIndex++) {
        ...
        const generatedLastCharacters = generatedCode.slice(-4);
        let takenGiftCard;
        ...
        const initialValue = cardItem.price_set.shop_money.amount;

        const newGiftCard = await STORE_TO_SYNC_FROM.request(giftCardCreate, {
          input: {
            code: generatedCode,
        ...
        if (newGiftCard.giftCardCreate?.userErrors.length > 0) {
          console.log("New gift card errors:", newGiftCard.giftCardCreate?.userErrors);
        }
        ...
        if (newGiftCard.giftCardCreate?.giftCard) {
          ...
          if (newGiftCardCopy.giftCardCreate?.userErrors.length > 0) {
            console.log("New gift card copy errors:", newGiftCardCopy.giftCardCreate?.userErrors);
          }

          if (newGiftCardCopy.giftCardCreate?.giftCard) {
            await GiftCard.create({
              last_characters: newGiftCard?.giftCardCreate?.giftCard?.lastCharacters,
              ...
              [stores.origin.giftCardColumnName]: newGiftCard?.giftCardCreate?.giftCard?.id,
              [stores.destination.giftCardColumnName]: newGiftCardCopy?.giftCardCreate?.giftCard?.id,
            });

            // if both gift cards are created and saved into DB, create fulfillment in Shopify
            const order = await STORE_TO_SYNC_FROM.request(orderQuery, {
              id: orderId,
            });
            let giftCardItems = [];

            for (const [index, fulfillmentOrder] of order?.order?.fulfillmentOrders?.edges.entries()) {
              let fulfillmentsOrderObj = {
                id: fulfillmentOrder?.node?.id,
                giftItems: [] as any[],
              };
              let items = fulfillmentOrder?.node?.lineItems?.edges?.filter((item: any) => item?.node?.lineItem?.variant?.sku?.includes("GIFTCARD"));
            ...
            giftCardItems = giftCardItems.filter((item: any) => item.giftItems.length > 0);

            if (!giftCardItems || giftCardItems?.length == 0) return;

            const lineItemsByFulfillmentOrderObj = giftCardItems.map((item: any) => {
              return {
            ...
            await STORE_TO_SYNC_FROM.request(fulfillGiftCardItems, {
              fulfillment: {
    ...
    return res.status(200).json(`GIFT CARD CREATE: Created gift cards for order ${orderName}`);
  } catch (error) {
